...
class MLP(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, drop=0., beta=[0.25, 0.44, 0.64, 0.99]):
        ...
        self.fc1_lif =  GaussianIntegrateAndFire(tau=2.0, detach_reset=True )

        self.fc2_linear = nn.Linear(hidden_features, out_features)
        self.fc2_bn = nn.BatchNorm1d(out_features)
        self.fc2_lif =  GaussianIntegrateAndFire(tau=2.0, detach_reset=True )
...
class SSA(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1, beta=[0.25, 0.44, 0.64, 0.99]):
        super().__init__()
        ...
        self.beta=beta
        self.q_lif = GaussianIntegrateAndFire(tau=2.0, detach_reset=True, beta=self.beta)

        self.k_linear = nn.Linear(dim, dim)
        self.k_bn = nn.BatchNorm1d(dim)
        self.k_lif = GaussianIntegrateAndFire(tau=2.0, detach_reset=True, beta=self.beta)

        self.v_linear = nn.Linear(dim, dim)
        self.v_bn = nn.BatchNorm1d(dim)
        self.v_lif = GaussianIntegrateAndFire(tau=2.0, detach_reset=True, beta=self.beta)
        self.attn_lif = GaussianIntegrateAndFire(tau=2.0, v_threshold=0.5, detach_reset=True, beta=self.beta)

        self.proj_linear = nn.Linear(dim, dim)
        self.proj_bn = nn.BatchNorm1d(dim)
        self.proj_lif = GaussianIntegrateAndFire(tau=2.0, detach_reset=True, beta=self.beta)
...
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., norm_layer=nn.LayerNorm, sr_ratio=1, beta=[0.25, 0.44, 0.64, 0.99]):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.beta=beta
        self.attn = SSA(dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale,
                              attn_drop=attn_drop, proj_drop=drop, sr_ratio=sr_ratio, beta=self.beta)
...
class SPS(nn.Module):
    def __init__(self, img_size_h=128, img_size_w=128, patch_size=4, in_channels=2, embed_dims=256, beta=[0.25, 0.44, 0.64, 0.99]):
        ...
        self.proj_lif =  GaussianIntegrateAndFire(tau=2.0, detach_reset=True )

        self.proj_conv1 = nn.Conv2d(embed_dims//8, embed_dims//4, kernel_size=3, stride=1, padding=1, bias=False)
        self.proj_bn1 = nn.BatchNorm2d(embed_dims//4)
        self.proj_lif1 =  GaussianIntegrateAndFire(tau=2.0, detach_reset=True )

        self.proj_conv2 = nn.Conv2d(embed_dims//4, embed_dims//2, kernel_size=3, stride=1, padding=1, bias=False)
        self.proj_bn2 = nn.BatchNorm2d(embed_dims//2)
        self.proj_lif2 =  GaussianIntegrateAndFire(tau=2.0, detach_reset=True )
        self.maxpool2 = torch.nn.MaxPool2d(kernel_size=3, stride=2, padding=1, dilation=1, ceil_mode=False)

        self.proj_conv3 = nn.Conv2d(embed_dims//2, embed_dims, kernel_size=3, stride=1, padding=1, bias=False)
        self.proj_bn3 = nn.BatchNorm2d(embed_dims)
        self.proj_lif3 =  GaussianIntegrateAndFire(tau=2.0, detach_reset=True )
        self.maxpool3 = torch.nn.MaxPool2d(kernel_size=3, stride=2, padding=1, dilation=1, ceil_mode=False)

        self.rpe_conv = nn.Conv2d(embed_dims, embed_dims, kernel_size=3, stride=1, padding=1, bias=False)
        self.rpe_bn = nn.BatchNorm2d(embed_dims)
        self.rpe_lif =  GaussianIntegrateAndFire(tau=2.0, detach_reset=True )
...
class Spikformer(nn.Module):
    def __init__(self,
                 ...
                 depths=[6, 8, 6], sr_ratios=[8, 4, 2], T = 4, beta=0.25
                 ):
        super().__init__()
        self.T = T  # time step
        ...
        block = nn.ModuleList([Block(
            ...
            norm_layer=norm_layer, sr_ratio=sr_ratios, beta=self.beta)
