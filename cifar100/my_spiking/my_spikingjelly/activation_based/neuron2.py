...
class GaussianIntegrateAndFire(BaseNode):
    def __init__(self, tau, 
                 ...
                 step_mode='m', backend='cupy', store_v_seq: bool = False,
                   e_exc= 40.0, 
                   e_inh= -40.0, 
                   g_exc= 1.0, 
                   g_inh= 1.0, 
                   beta= [0.25, 0.44, 0.64, 0.99],
                   epsilon = torch.rand_like(torch.empty(8, 3, 32, 32)),
                   device='cuda'):
        assert isinstance(tau, float) and tau > 1.
        super().__init__()
        self.tau = tau
        ...
        self.g_inh = g_inh
        # self.beta= beta
        self.beta = lambda view_dims : (torch.tensor(beta).cuda()).view(*view_dims)
        self.alpha = lambda view_dims : (1 - torch.tensor(beta).cuda()).view(*view_dims)
        self.epsilon = lambda x : torch.rand_like(x)
        init_w = - math.log(tau - 1.)
        self.w = nn.Parameter(torch.as_tensor(init_w))
        # self.step_mode= 'm'
        # self.backend = 'cupy'

        # self.register_buffer('membrane_potential', torch.tensor([self.v_reset]))
    ...
    def neuronal_charge(self, x: torch.Tensor):
        ...
        # self.epsilon = torch.rand_like(torch.empty(x.shape)).cuda()
        # print(x.shape)
        # print(self.beta.shape)
        # print(self.epsilon.shape)
        # print("NNNNNNN",x.dtype)

        num_dims = len(x.shape)
        view_dims = [4] + [1] * (num_dims - 1)
        # print(self.beta)
        if self.decay_input:
            if self.v_reset is None or self.v_reset == 0.:
                # print(self.v)
                # self.v = self.v + (x - self.v + self.sigma * self.tau) / self.tau 
                # self.v = self.v + (x - self.v + self.sigma * self.tau * self.epsilon) / self.tau 
                # self.v = self.v + ((1 - self.beta) * x - self.v + self.g_exc * (self.e_exc - self.v) + self.g_inh * (self.e_inh - self.v) + self.beta * self.tau * self.epsilon(x)) / self.tau 
                self.v = self.v + ((self.alpha(view_dims) * x) - self.v + self.g_exc * (self.e_exc - self.v) + self.g_inh * (self.e_inh - self.v) + self.tau * (self.beta(view_dims) * self.epsilon(x))) / self.tau 
                # self.v = self.v + ((torch.sqrt(1 - torch.tensor(self.beta).cuda()).view(*view_dims) * x) - self.v + self.g_exc * (self.e_exc - self.v) + self.g_inh * (self.e_inh - self.v) + self.tau * (torch.sqrt(torch.tensor(self.beta).cuda()).view(*view_dims) * self.epsilon)) / self.tau 
                # print(self.v)
    

            else:
                # self.v = self.v + (x - (self.v - self.v_reset) + self.sigma * self.tau) / self.tau
                # self.v = self.v + (x - (self.v - self.v_reset) + self.sigma * self.tau * self.epsilon) / self.tau
                self.v = self.v + ((1 - self.beta) * x - (self.v - self.v_reset) + self.g_exc * (self.e_exc - self.v) + self.g_inh * (self.e_inh - self.v) + self.beta * self.tau * self.epsilon) / self.tau
                # self.v = self.v + ((torch.sqrt(1 - torch.tensor(self.beta).cuda()).view(*view_dims) * x) - (self.v - self.v_reset) + self.g_exc * (self.e_exc - self.v) + self.g_inh * (self.e_inh - self.v) + self.tau * (torch.sqrt(torch.tensor(self.beta).cuda()).view(*view_dims) * self.epsilon)) / self.tau
        else:
            if self.v_reset is None or self.v_reset == 0.:
                # self.v = self.v +(- (self.v) + self.sigma * self.tau) / self.tau + x
                # self.v = self.v +(- (self.v) + self.sigma * self.tau * self.epsilon) / self.tau + x
                self.v = self.v +(- (self.v) + self.g_exc * (self.e_exc - self.v) + self.g_inh * (self.e_inh - self.v) + self.beta * self.tau * self.epsilon) / self.tau + (1 - self.beta) * x
                # self.v = self.v +(- (self.v) + self.g_exc * (self.e_exc - self.v) + self.g_inh * (self.e_inh - self.v) + self.tau * (torch.sqrt(torch.tensor(self.beta).cuda()).view(*view_dims) * self.epsilon)) / self.tau + (torch.sqrt(1 - torch.tensor(self.beta).cuda()).view(*view_dims) * x)
            else:
                # self.v = self.v +(- (self.v - self.v_reset) + self.sigma * self.tau) / self.tau + x
                # self.v = self.v +(- (self.v - self.v_reset) + self.sigma * self.tau * self.epsilon) / self.tau + x
                self.v = self.v +(- (self.v - self.v_reset) + self.g_exc * (self.e_exc - self.v) + self.g_inh * (self.e_inh - self.v) + self.beta * self.tau * self.epsilon) / self.tau + (1 - self.beta) * x
                # self.v = self.v +(- (self.v - self.v_reset) + self.g_exc * (self.e_exc - self.v) + self.g_inh * (self.e_inh - self.v) + self.tau * (torch.sqrt(torch.tensor(self.beta).cuda()).view(*view_dims) * self.epsilon)) / self.tau + (torch.sqrt(1 - torch.tensor(self.beta).cuda()).view(*view_dims) * x)
