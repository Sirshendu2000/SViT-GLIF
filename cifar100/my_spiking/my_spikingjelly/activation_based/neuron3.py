...
#                    e_inh=-70.0, 
#                    sigma=0.25,
#                    epsilon=torch.rand_like(8, 3, 32, 32),
#                    device='cuda'):
#         assert isinstance(tau, float) and tau > 1.
#         super().__init__()
#         self.tau = tau
...
class GaussianIntegrateAndFire(BaseNode):
    def __init__(self, tau, 
        ...
        self.sigma = lambda x : torch.std(x)
    ...
    def neuronal_charge(self, x: torch.Tensor):
        ...
        if self.decay_input:
            if self.v_reset is None or self.v_reset == 0.:
                # self.v = self.v + (x - self.v + self.sigma * self.tau) / self.tau 
                # self.v = self.v + (x - self.v + self.sigma * self.tau * self.epsilon) / self.tau 
                self.v = self.v + (x - self.v  + self.beta * self.sigma(x) * self.epsilon(x)) / self.tau 
            else:
                # self.v = self.v + (x - (self.v - self.v_reset) + self.sigma * self.tau) / self.tau
                # self.v = self.v + (x - (self.v - self.v_reset) + self.sigma * self.tau * self.epsilon) / self.tau
                self.v = self.v + (x - (self.v - self.v_reset) + self.g_exc * (self.e_exc - self.v) + self.g_inh * (self.e_inh - self.v) + self.beta * torch.std(x) * self.epsilon(x)) / self.tau
        else:
            if self.v_reset is None or self.v_reset == 0.:
                # self.v = self.v +(- (self.v) + self.sigma * self.tau) / self.tau + x
                # self.v = self.v +(- (self.v) + self.sigma * self.tau * self.epsilon) / self.tau + x
                self.v = self.v +(- (self.v) + self.g_exc * (self.e_exc - self.v) + self.g_inh * (self.e_inh - self.v) + self.beta * torch.std(x) * self.epsilon(x)) / self.tau + x
            else:
                # self.v = self.v +(- (self.v - self.v_reset) + self.sigma * self.tau) / self.tau + x
                # self.v = self.v +(- (self.v - self.v_reset) + self.sigma * self.tau * self.epsilon) / self.tau + x
                self.v = self.v +(- (self.v - self.v_reset) + self.g_exc * (self.e_exc - self.v) + self.g_inh * (self.e_inh - self.v) + self.beta * torch.std(x) * self.epsilon(x)) / self.tau + x
    ...
    def multi_step_forward(self, x_seq: torch.Tensor):
        if self.backend == 'torch':
            ...
            spike_seq, v_seq = ac_neuron_kernel.ParametricLIFNodeATGF.apply(
                x_seq.flatten(1), self.v.flatten(0), self.v_threshold, self.v_reset, self.tau.to(x_seq),
                self.forward_kernel, self.backward_kernel)
